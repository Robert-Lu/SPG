...
def _model_load(model, pretrained_dict):
    ...
    if list(model_dict.keys())[0].startswith('module.'):
        pretrained_dict = {'module.'+k: v for k, v in list(pretrained_dict.items())}
...
def _model_load_v6(model, pretrained_dict):
    ...
    common_pred = {k: v for k, v in list(pretrained_dict.items()) if k in list(model_dict.keys())}
    print("Weights cannot be loaded:")
    print([k for k in list(model_dict.keys()) if k not in list(common_pred.keys())+ list(feature2_pred_w.keys()) + list(feature2_pred_b.keys())])
...
def _model_load_v2(model, pretrained_dict):
    ...
    common_pred = {k: v for k, v in list(pretrained_dict.items()) if k in list(model_dict.keys())}
    print("Weights cannot be loaded:")
    print([k for k in list(model_dict.keys()) if k not in list(common_pred.keys())+list(fc5_cls_w.keys())+
           list(fc5_cls_b.keys()) + list(fc5_seg_w.keys()) + list(fc5_seg_b.keys())])
